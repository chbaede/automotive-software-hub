...
export const TechDetailDrawer: React.FC<TechDetailDrawerProps> = ({
  ...
  const connectedPaths = useMemo(() => {
    ...
    return pathsByTechnologyId.get(technology.id) || [];
  }, [technology]);
  ...
  }, [technology, discoveryResult, connectedPaths]);
              ...
              <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-200 dark:border-slate-800 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5 text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">
                    ...
                    <span>{t.techDetail.evidenceAndTrust}</span>
                  </div>
                  {fs.lastVerified && (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                  ...
                  <div className="p-2.5 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 space-y-0.5">
                    <div className="text-[10px] font-mono text-slate-500 uppercase font-semibold">{t.techDetail.standardLabel}</div>
                    <div className="font-extrabold text-slate-900 dark:text-slate-100">{fs.standard || 'ISO 26262'}</div>
                  </div>
                  <div className="p-2.5 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 space-y-0.5">
                    <div className="text-[10px] font-mono text-slate-500 uppercase font-semibold">{t.techDetail.evidenceLabel}</div>
                    {fs.sourceUrl ? (
                      <a
                        href={fs.sourceUrl}
                        target="_blank"
                        ...
                        title={fs.sourceUrl}
                      >
                        <span className="truncate">{t.techDetail.datasheetSpec}</span>
                        <ExternalLink className="w-2.5 h-2.5 shrink-0" />
                      </a>
                    ) : (
                      <span className="font-bold text-slate-500">{t.techDetail.documented}</span>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-3 bg-gradient-to-r from-indigo-500/10 via-purple-500/5 to-slate-100 dark:to-slate-950 p-4 rounded-xl border border-indigo-500/30">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider">
                  ...
                  <span>{t.techDetail.stackPaths} ({connectedPaths.length})</span>
                </div>
                <span className="text-[10px] font-mono text-slate-500">{t.techDetail.architectureJourney}</span>
              </div>
