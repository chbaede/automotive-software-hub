...
export const GraphPathFinder: React.FC<GraphPathFinderProps> = ({
  ...
  // Compute BFS Shortest Path with optional relationship filtering
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-6 transition-colors">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
              ...
              <span className="text-xs px-2 py-0.5 rounded-full bg-cyan-500/10 dark:bg-cyan-500/20 text-cyan-700 dark:text-cyan-300 font-normal border border-cyan-500/30">
      ...
      {/* Selector Controls */}
      <div className="grid grid-cols-1 md:grid-cols-[1fr,auto,1fr] gap-3 items-center bg-slate-50 dark:bg-slate-950/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800/80">
        {/* Source Dropdown */}
        <div>
          <label className="block text-xs font-semibold text-slate-700 dark:text-slate-400 mb-1.5 flex items-center gap-1.5">
          ...
          <select
            ...
            className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-cyan-500 shadow-2xs"
          >
        ...
        <div className="flex justify-center pt-5">
          <button
            ...
            className="p-2.5 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-slate-700 transition-colors shadow-2xs"
          >
        ...
        <div>
          ...
          <select
            ...
            className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-500 shadow-2xs"
          >
            {sortedTechs.map((tech) => (
              <option key={tech.id} value={tech.id}>
                {tech.name} ({getLayerName(tech.layerId)})
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-50 dark:bg-slate-950/40 p-3 rounded-xl border border-slate-200 dark:border-slate-800/60 text-xs">
        {/* Example Paths */}
        <div className="flex flex-wrap items-center gap-1.5">
          <span className="text-slate-600 dark:text-slate-400 font-semibold">{language === 'ko' ? '탐색 예시:' : 'Example Paths:'}</span>
          {EXAMPLE_PATHS.map((preset, idx) => (
            <button
              ...
              className="px-2.5 py-1 rounded-lg bg-white dark:bg-slate-800 hover:bg-cyan-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 hover:text-cyan-700 dark:hover:text-white border border-slate-200 dark:border-slate-700/60 transition-colors text-xs shadow-2xs"
            >
      ...
      {pathResult && (
        <div className="space-y-4 pt-1">
          {pathResult.found ? (
            <div>
              {/* Path Summary Metric */}
              <div className="flex items-center justify-between bg-cyan-50 dark:bg-cyan-950/30 border border-cyan-200 dark:border-cyan-800/40 rounded-xl px-4 py-2.5 text-xs text-cyan-800 dark:text-cyan-300 mb-4">
                <div className="flex items-center gap-2">
              ...
              <div className="space-y-3">
                {pathResult.steps.map((step: GraphPathStep, idx: number) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800/90 rounded-xl p-4 transition-all hover:border-slate-300 dark:hover:border-slate-700 space-y-3"
                    >
                      {/* Step Header */}
                      ...
                      {/* Node to Node row */}
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-slate-900/90 p-3 rounded-lg border border-slate-200 dark:border-slate-800">
                        {/* Source Tech */}
                        <button
                        ...
                        >
                          ...
                          <div className="font-semibold text-slate-900 dark:text-white group-hover:text-cyan-600 dark:group-hover:text-cyan-400 flex items-center gap-1 mt-0.5 text-sm">
                        ...
                        >
                          <div className="text-xs text-slate-500 dark:text-slate-400 flex items-center sm:justify-end gap-1.5">
                            <Layers className="w-3 h-3 text-slate-400 dark:text-slate-500" />
                            {toLayer}
                          </div>
                          <div className="font-semibold text-slate-900 dark:text-white group-hover:text-cyan-600 dark:group-hover:text-cyan-400 flex items-center sm:justify-end gap-1 mt-0.5 text-sm">
                      ...
                      <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed bg-white dark:bg-slate-900/50 p-2.5 rounded-lg border border-slate-200 dark:border-slate-800/60">
                        💬 {relDesc}
                      </p>
                    </div>
                  );
            ...
            <div className="flex items-center gap-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/40 rounded-xl p-4 text-amber-800 dark:text-amber-300 text-xs">
          ...
          <div className="flex items-start gap-2 bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 rounded-xl p-3 text-[11px] text-slate-500 dark:text-slate-400">
